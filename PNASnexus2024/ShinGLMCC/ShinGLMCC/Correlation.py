...
##  [input]
##   +"dataid": (str) Folder name where the original npy format files stored.
##  [output]
##   +"dicspk": (dict)
##    {
##     *key:   (int) Neuron ID.
...
##   +"dicspkr": (dict)
##    {
##     *key:   (int) Renumbered ID (Sequential number from 0).
##     *value: (list of float) Spike times of each neuron.
##    }
...
def npx2dic(dataid):
    ...
    ## Merge clusters.npy and times.npy
    dfspk = pd.DataFrame(np.concatenate([npcls,nptime],1),
                         columns=["cls","time"])
  
    ## Trim spikes over REC_MS
    idxspnum = np.argmax(nptime>=REC_MS)
    dfspk = dfspk[:idxspnum]

    ## Make "good" group lst
    lscls = dfgrp.query("group=='good'")["cls"].tolist()

    ## Make a dict of spike time lists with cls key
    grspk = dfspk.groupby("cls")
    dicspk = {}
    dicspkr = {}

    for n in range(len(lscls)):
        dicspk[lscls[n]] = (grspk.get_group(lscls[n])["time"]
                            .sort_values().values.tolist())
        dicspkr[n] = (grspk.get_group(lscls[n])["time"]
                      .sort_values().values.tolist())

    return dicspk,dicspkr
