...
BETA = {"sGLM":2410000,"cGLM":4000,"cShin":1000000}
...
def _calc_nlogpost_grad(mode, p, hsK, hsJ, fn, beta, dof):
    ...
    if mode=="Shin":
        ...
        ## In the paper, the component of the log-prior was weaken
        ## by inverting the sign.
        ## a_2dfK = -a_2dfK
...
def cor2glm(mode,dfccj,dfcck,dof,d=1,t=2,*args):

    ## Get the firts neuron pairs (small idx to large idx)
    dfhccj = dfccj.query("ref<tar").reset_index(drop=True)
    dfhcck = dfcck.query("ref<tar").reset_index(drop=True)

    ## Get neuron pair idx list ----------------------------
    lscls = dfhcck[["ref","tar"]].values.tolist()

    ## Extract data for GLMCC estimation -------------------
    w = WINHALF_MS["C"]
    dfj = dfhccj.T[:-2].query("-@w<=index<=@w").T.reset_index(drop=True)
    dfk = dfhcck.T[:-2].query("-@w<=index<=@w").T.reset_index(drop=True)
    ...
    jout = joblib.Parallel(n_jobs=PROCESS_NUM)(
        [joblib.delayed(ShinGLMCC)(
            mode,npa[n],np.array(dfk.iloc[n]),np.array(dfj.iloc[n]),fnj,beta,dof)
         for n in range(len(lscls))])

    npa, dfa, dfl =Joblib2Para(jout, lscls, dfk.columns)

    if dof==2:
        dfdt = pd.DataFrame([[d,t]]*len(dfl),columns=["delay","tau"])
        dfl = pd.concat([dfl,dfdt],axis=1)
